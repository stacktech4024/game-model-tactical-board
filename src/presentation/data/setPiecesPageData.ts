...
const wideFreeKickSteps: PreviewStep[] = [
  {
    id: 'wf-set',
    cue: 'SET — #4/#5/#9/#10/#11 hold one connected line against a hybrid defensive structure.',
  ...
  {
    id: 'wf-separation',
    cue: 'SEPARATE — staggered checks free near, central, and far-post lanes while every runner stays onside.',
    playerMoves: [
      { playerId: 'wf-home-4', to: { x: 38, y: 24 } },
      { playerId: 'wf-home-10', to: { x: 44, y: 23 }, startDelay: 0.04 },
      { playerId: 'wf-home-9', to: { x: 50, y: 22 }, startDelay: 0.08 },
      { playerId: 'wf-home-5', to: { x: 56, y: 24 }, startDelay: 0.12 },
      { playerId: 'wf-home-11', to: { x: 62, y: 23 }, startDelay: 0.16 },
      { playerId: 'wf-away-3', to: { x: 38, y: 25 }, startDelay: 0.08 },
      { playerId: 'wf-away-6', to: { x: 50, y: 24 }, startDelay: 0.14 },
      { playerId: 'wf-away-10', to: { x: 61, y: 25 }, startDelay: 0.2 },
    ],
    duration: 0.48,
  },
  {
    id: 'wf-delivery',
    cue: 'DELIVERY — #9 attacks the selected central corridor as near, far, and decoy runs separate the line.',
    ballFrom: { x: 91, y: 42 },
    ballTo: { x: 49, y: 13 },
    playerMoves: [
      { playerId: 'wf-home-4', to: { x: 37, y: 14 } },
      { playerId: 'wf-home-10', to: { x: 43, y: 17 }, startDelay: 0.05 },
      { playerId: 'wf-home-9', to: { x: 49, y: 13 }, startDelay: 0.09 },
      { playerId: 'wf-home-5', to: { x: 57, y: 19 }, startDelay: 0.13 },
      { playerId: 'wf-home-11', to: { x: 62, y: 14 }, startDelay: 0.18 },
      { playerId: 'wf-away-3', to: { x: 38, y: 17 }, startDelay: 0.08 },
      { playerId: 'wf-away-6', to: { x: 50, y: 17 }, startDelay: 0.15 },
      { playerId: 'wf-away-10', to: { x: 61, y: 18 }, startDelay: 0.22 },
  ...
  {
    id: 'wf-first-contact',
    cue: 'FIRST CONTACT — #9 attacks the targeted ball under pressure; the contested clearance drops toward #8.',
    ballFrom: { x: 49, y: 13 },
    ballTo: { x: 48, y: 39 },
  ...
  {
    id: 'wf-second-ball',
    cue: 'SECOND BALL — #8 secures the clearance and resets through #6 while #2/#3 preserve rest defence.',
    ballFrom: { x: 48, y: 39 },
    ballTo: { x: 55, y: 53 },
    playerId: 'wf-home-6',
    playerTo: { x: 55, y: 53 },
...
const wideFreeKickRoutes: PreviewRoute[] = [
  ...
  { id: 'wf-near-run', from: { x: 39, y: 32 }, to: { x: 37, y: 14 }, type: 'run', revealOnStepId: 'wf-delivery' },
  { id: 'wf-cross-run', from: { x: 57, y: 31 }, to: { x: 43, y: 17 }, type: 'run', revealOnStepId: 'wf-delivery' },
  { id: 'wf-primary-run', from: { x: 51, y: 32 }, to: { x: 49, y: 13 }, type: 'run', revealOnStepId: 'wf-delivery' },
  { id: 'wf-decoy-run', from: { x: 45, y: 31 }, to: { x: 57, y: 19 }, type: 'run', revealOnStepId: 'wf-delivery' },
  { id: 'wf-far-run', from: { x: 63, y: 32 }, to: { x: 62, y: 14 }, type: 'run', revealOnStepId: 'wf-delivery' },
  { id: 'wf-delivery-ball', from: { x: 91, y: 42 }, to: { x: 49, y: 13 }, type: 'pass', revealOnStepId: 'wf-delivery' },
  { id: 'wf-clearance', from: { x: 49, y: 13 }, to: { x: 48, y: 39 }, type: 'pass', revealOnStepId: 'wf-first-contact' },
...
export const SET_PIECES_PAGE_CASES: SetPiecePageCase[] = [
  {
    ...
    strategy: 'Use a legal short-angle trigger and late cluster separation to target #9 in the central six-yard corridor while preserving the second phase.',
    tactics: [
      '#3 receives short while the connected group holds until the new delivery cue',
      '#4/#10 clear near space; #9 attacks the selected central corridor',
      '#5 manipulates a marker and #11 attacks the secondary far-post lane',
      '#8 protects the second ball; #2/#6 remain connected against the counter',
    ],
    skillSet: ['Delivery', 'Disguise', 'Acceleration', 'Run timing', 'Aerial timing', 'Heading', 'Communication', 'Second-ball reaction'],
    principles: ['MOBILITY', 'PENETRATION', 'IMPROVISATION', 'SUPPORT'],
    caption: 'Short-angle trigger, connected back-side cluster, staggered three-zone release, targeted first contact, contested clearance, second-ball control, and rest defence.',
  ...
  },
  {
    ...
    organization: '#7 delivers from wide Channel 1; #4/#5/#9/#10/#11 form a connected line, #8 owns the second ball, and #2/#3/#6 hold security. The opponent uses a hybrid of priority-space zones, matched aerial threats, a short watcher, and edge protection.',
    strategy: 'From a crossing angle that needs no shooting wall, release the compact line on the kicker\'s cue, separate the hybrid defence, and target #9 centrally.',
    tactics: [
      '#2 shows short while the five-player line holds its starting height',
      '#4/#10 clear near space; #9 attacks the selected central delivery',
      '#5 manipulates a marker and #11 arrives later into the far-post lane',
      '#8 protects the second ball; #2/#3/#6 remain connected for the reset',
    ],
    skillSet: ['Timing', 'Acceleration', 'Disguise', 'Delivery quality', 'Heading', 'Body orientation', 'Communication', 'Second-ball anticipation'],
    principles: ['MOBILITY', 'PENETRATION', 'IMPROVISATION', 'SUPPORT'],
    caption: 'Connected setup, staggered line release, targeted central delivery, contested first contact, second-ball control, and secure rest defence.',
